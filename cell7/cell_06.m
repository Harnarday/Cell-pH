...
phi_H           = 1*10^-5;
psi_H           = 1*10^-5;      % constant for H flux on the vent side
phi_OH          = 1*10^-5;


% Change in H inside the cell
dx(1) = (phi_H * (H_ocean - H_cell)) + (psi_H * (H_vent - H_cell)) * dt;
% here we are now including the efflux of H into the vent
% current unit is mol/ dm^3

%change in OH
dx(2) = phi_OH * (OH_vent - OH_cell) * dt;
